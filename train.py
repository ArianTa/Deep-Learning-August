...
def main(**kwargs):
    ...
    train_data = dataset(root = data_dir, 
...
if __name__ == "__main__":
    ...
    # Optimizer might depend on the model's architecture, hence we get them at the same time
    if 'resnet' in  args.model:
        model, optimizer = get_resnet_model(args.model, "Adam", args.lr, len(os.listdir(args.path)))
    # else blabla

    main(
        data_dir = args.path,
        device = torch.device("cuda:0" if torch.cuda.is_available() and args.gpu == 'y' else "cpu"),
        batch_size = args.batch,
        epochs = args.epochs,
        model = model,
        optimizer = optimizer,
        criterion = criterion,
        train_transforms = locate("pytorch_transforms." + args.transforms + ".train_transforms"),
        test_transforms = locate("pytorch_transforms." + args.transforms + ".test_transforms"),
